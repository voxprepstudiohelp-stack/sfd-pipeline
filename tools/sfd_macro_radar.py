...
부스트 로직:
- 환율 강세/약세 → 섹터 수혜/피해 매핑
- 원자재 급등 → 관련 기업 부스트 (±15pt)
- VIX > 25 → 공포 구간 게이팅
- BTC +10% (5일 누적) → 코스닥 성장주 +pt
"""
...
INDICATORS = {
    ...
    "FED_RATE": "^IRX",      # Fed Funds Rate (3M Treasury 대체)
}
...
SECTOR_MACRO_MAP = {
    ...
    "ROBOTICS": {
        ...
        "DXY_weak":     -4,
        "OIL_spike":    -3,
        "COPPER_spike":  8,   # 구리/금속 소재 수혜
        "BTC_rally":    10,   # Risk-on → 성장주 수혜
        "VIX_fear":    -12,
    },
}
...
def fetch_macro_indicators(lookback_days=5):
    ...
    for key, ticker in INDICATORS.items():
        try:
            ...
            logger.info(f"{key:12s}: {current_price:10.4f} ({change_pct:+7.2f}%)")
...
def generate_macro_signals(macros):
    ...
    Returns:
        dict: {"DXY_signal": "STRONG"|"WEAK"|"NEUTRAL", "OIL_spike": bool, ...}
    """
    signals = {}
    ...
    signals["LIT_spike"] = lit_change < -3.0

    # JPY 급등 (+2% = 엔화 캐리 청산 신호 → 전체 약세)
    jpy_change = macros.get("JPY", {}).get("change_%", 0)
    signals["JPY_squeeze"] = jpy_change > 2.0
...
def calculate_sector_boosts(signals, macros):
    """
    섹터별 boost_score 산출 (캡핑: ±15pt)

    Returns:
        dict: {"SEMICONDUCTOR": {"boost": 8, "reasons": [...]}, ...}
    """
    boosts = {}
    ...
    for sector, sensitivities in SECTOR_MACRO_MAP.items():
        ...
        if signals["OIL_spike"]:
            ...
            reasons.append(f"OIL {sensitivities['OIL_spike']:+d}pt (급등)")

        # 구리 급등
        if signals["COPPER_spike"]:
        ...
        if signals["LIT_spike"] and sector in ["MATERIALS", "ENERGY"]:
            boost_score -= 8
            reasons.append("LIT -8pt (배터리 수요 둔화)")

        # JPY 급등 (캐리 청산 → 전반적 약세)
        if signals["JPY_squeeze"]:
            boost_score -= 5
            reasons.append("JPY -5pt (캐리 청산 위험)")
...
def main():
    logger.info("=" * 60)
    logger.info("SFD MACRO RADAR v1.0 - 시작")
    logger.info("=" * 60)
