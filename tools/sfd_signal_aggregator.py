# -*- coding: utf-8 -*-
# sfd_signal_aggregator.py | v4.1 | Claude (Anthropic) 2026-06-15
# Deploy to: sfd-pipeline/tools/sfd_signal_aggregator.py
#
# [v4.0 -> v4.1 changes]
# - [BM-WAIST] compute_waist_bias() 연동: 멀티lookback[10,20,40] 중위값 허리 산출
#   -> BLOCK 시 BUY 계열 BM 점수 전량 억제 (apply_waist_gate)
#   -> guardian_warn: waist_pct < -3% 시 Guardian 경고 플래그
# - [BM-3HIT]  detect_3hit_breakout() 연동: 저항선 3회 터치 후 돌파 +10pt
# - [BM-AFTRAP] detect_af_trap() 연동: 전저점 얕은 이탈 후 회복 +12pt
#   -> 선결조건: waist_bias == "LONG" (하락추세 AF 트랩 배제)
#   -> target_multiplier 1.5x 출력 (kis_trader 연동용)
# - results 신규 컬럼: waist_bias / waist_pct_v2 / guardian_warn /
#                      bm_3hit_signal / bm_3hit_score /
#                      bm_aftrap_signal / bm_aftrap_score / bm_target_mult / waist_blocked
#
# [v3.9 -> v4.0 changes]
...
#  investor_score     최대  +35pt  (외국인 +10 / 기관 +10 / BM-19 연금 +15)
#  theme_score        최대  +20pt
#  BM-10 vol_surge    최대  +15pt
#  BM-12 pullback     최대  +10pt
#  BM-18 alpha_decay  최대  +10pt
#  BM-3HIT            최대  +10pt  [v4.1 신규]
#  BM-AFTRAP          최대  +12pt  [v4.1 신규]
# ══════════════════════════════════════════════════════
...
# [v4.1] 신규 BM 모듈
try:
    from sfd_bm_waist   import compute_waist_bias, apply_waist_gate
    from sfd_bm_3hit    import detect_3hit_breakout
    from sfd_bm_af_trap import detect_af_trap
    BM_NEW_AVAILABLE = True
...
ALPHA_DECAY_CSV          = os.path.join(LATEST_DIR, "bm18_alpha_decay.csv")
...
def score_investor(ticker, investor_df):
    ...
    - [BM-19] 외국인+기관+연금 동시: +15pt 보너스 (최대 +35pt)
...
def load_decay_score_map() -> dict:
    ...
    try:
        ...
        logging.info(f"[BM-18] decay_map: {len(result)} rows")
        return result
    except Exception as e:
        logging.warning(f"[BM-18] decay_map load failed: {e}")
        return {}


# ── [BM-19] DART event map ───────────────────────────────────────────────────
def load_dart_event_map() -> dict:
    ...
    try:
        ...
        logging.warning(f"[BM-19] dart_event_map load failed: {e}")
    return result
...
def apply_signal_timeout(ticker, raw_signal, trade_date, prev_state) -> tuple:
    ticker_state = prev_state.get(ticker)
    if raw_signal in TIMEOUT_SIGNALS:
        new_state = {"signal": raw_signal, "issued_date": trade_date, "bars_elapsed": 0}
...
def load_tech_detail_map() -> dict:
    if not os.path.exists(TECH_DETAIL_CSV):
        logging.warning(f"[v4.1] TECH_DETAIL_CSV not found")
        return {}
    try:
        ...
        for _, row in df.iterrows():
            ...
            pb_score = float(row.get("pullback_zone_score", 0)) \
                       if has_pb and not pd.isna(row.get("pullback_zone_score")) else 0.0
            vs_score = float(row.get("vol_surge_score", 0)) \
                       if has_vs and not pd.isna(row.get("vol_surge_score")) else 0.0
            ...
            if _tcol and not pd.isna(row.get(_tcol)):
                ...
                if has_pb and pb_score > 0 and effective_tech <= 65.0:
                    ...
                    tech_source_ver = ("L2.7_v1.3" if has_vs
                                       else "L2.7_v1.2" if has_pb
                                       else "L2.7_v1.1")
            else:
        ...
        logging.info(f"[v4.1] tech_map: {len(tech_map)} tickers")
        return tech_map
    except Exception as e:
        logging.error(f"[v4.1] tech_map load failed: {e}")
        return {}
...
def load_news_score_map() -> dict:
    if not os.path.exists(NEWS_SCORE_CSV):
        logging.warning(f"[v4.1] NEWS_SCORE_CSV not found: {NEWS_SCORE_CSV}")
        return {}
    try:
        ...
        logging.info(f"[v4.1] news_score_map: {len(result)} rows")
        return result
    except Exception as e:
        logging.warning(f"[v4.1] news_score_map load failed: {e}")
        return {}
...
def load_fund_score_map() -> dict:
    ...
    if os.path.exists(FUNDAMENTAL_CSV_PRIMARY):
        ...
        logging.info(f"[v4.1] fund CSV (primary): {fund_csv}")
    elif os.path.exists(FUNDAMENTAL_CSV_FALLBACK):
        fund_csv = FUNDAMENTAL_CSV_FALLBACK
        logging.info(f"[v4.1] fund CSV (fallback): {fund_csv}")
    else:
        logging.warning(f"[v4.1] fundamental CSV not found")
        return {}
    try:
        ...
        logging.info(f"[v4.1] fund_score_map: {len(result)} rows | nonzero={nonzero}")
        return result
    except Exception as e:
        logging.warning(f"[v4.1] fund_score_map load failed: {e}")
        return {}


# ── [v4.0] Trend filter map ──────────────────────────────────────────────────
def load_trend_filter_map() -> dict:
    if not os.path.exists(TREND_FILTER_CSV):
        logging.info(f"[v4.0] trend_filter CSV not found — graceful skip")
        return {}
    try:
        ...
        for _, row in df.iterrows():
            result[str(row["ticker"])] = {
                "trend_pass":   str(row.get("trend_pass",   "PASS")).upper(),
                "trend_reason": str(row.get("trend_reason", "")),
            }
        logging.info(f"[v4.0] trend_filter_map: {len(result)} rows")
        return result
    except Exception as e:
        logging.warning(f"[v4.0] trend_filter_map load failed: {e}")
        return {}


# ── [v4.1] BM 기본값 헬퍼 ────────────────────────────────────────────────────
def _default_waist():
    return {"bias": "LONG", "waist_pct": 0.0, "guardian_warn": False, "score_delta": 0}

def _default_3hit():
    return {"signal": "NO_SIGNAL", "score_delta": 0, "touch_count": 0,
            "sideways_ratio": 0.0, "breakout_confirmed": False}

def _default_aftrap():
    return {"signal": "NO_SIGNAL", "score_delta": 0, "target_multiplier": 1.0}


# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    logging.info("=== sfd_signal_aggregator v4.1 START ===")
    ...
    if BM_NEW_AVAILABLE:
        ...
        logging.warning(f"[v4.1] BM-NEW import failed: {_BM_IMPORT_ERR} — scores set to 0")
    ...
    if investor_df is not None:
        logging.info(f"[v4.1] investor_df: {len(investor_df)} rows, cols: {list(investor_df.columns)}")
    else:
        logging.warning(f"[v4.1] INVESTOR_CSV not found: {INVESTOR_CSV}")
    ...
    decay_map         = load_decay_score_map()
    dart_map          = load_dart_event_map()
    trend_filter_map  = load_trend_filter_map()

    use_tech_detail = len(tech_detail_map) > 0
    logging.info(
        f"[v4.1] use_tech_detail={use_tech_detail} | tech={len(tech_detail_map)} "
        ...
        f"| timeout_tracked={len(timeout_state)} | trend_filter={len(trend_filter_map)}"
    )
    ...
    for ticker in tickers:
        ...
        # ── [v4.1] BM-WAIST / 3HIT / AF_TRAP ────────────────────────────────
        waist_result  = _default_waist()
        hit3_result   = _default_3hit()
        aftrap_result = _default_aftrap()
        ...
        if BM_NEW_AVAILABLE:
            try:
                ...
                if ohlcv_df is not None:
                    ...
                    hit3_result   = detect_3hit_breakout(ohlcv_df)
                    aftrap_result = detect_af_trap(ohlcv_df, waist_bias=waist_bias)
            except Exception as _bm_e:
                logging.debug(f"[v4.1] BM-NEW skip {ticker}: {_bm_e}")

        # Waist gate 적용 (BLOCK 시 BUY 계열 BM 점수 억제)
        bm_raw = {
        ...
        # [v4.0] trend_filter 후처리
        tf_data      = trend_filter_map.get(ticker, {})
        trend_pass   = tf_data.get("trend_pass",   "PASS")
        trend_reason = tf_data.get("trend_reason", "")
        if trend_pass == "FAIL" and timeout_signal in ("RESERVE_BUY", "WATCH_ONLY"):
            timeout_signal = "TREND_FAIL"
        ...
        results.append({
            ...
            "trend_filter_reason":  trend_reason,
            "tech_ver":             tech_ver,
            # ── [v4.1] 신규 컬럼 ─────────────────────────────────────────────
            "waist_bias":           waist_result.get("bias",          "LONG"),
            "waist_pct_v2":         waist_result.get("waist_pct",     0.0),
            "guardian_warn":        waist_result.get("guardian_warn", False),
            "bm_3hit_signal":       hit3_result.get("signal",         "NO_SIGNAL"),
            "bm_3hit_score":        bm_3hit_score,
            "bm_aftrap_signal":     aftrap_result.get("signal",       "NO_SIGNAL"),
            "bm_aftrap_score":      bm_aftrap_score,
            "bm_target_mult":       aftrap_result.get("target_multiplier", 1.0),
            "waist_blocked":        bm_gated.get("_waist_blocked",   False),
        })
    ...
    bm3hit_ct    = len(df_out[df_out["bm_3hit_score"] > 0])
    aftrap_ct    = len(df_out[df_out["bm_aftrap_score"] > 0])
    waist_blk_ct = len(df_out[df_out["waist_blocked"] == True])
    gw_ct        = len(df_out[df_out["guardian_warn"] == True])

    logging.info(
        f"=== DONE | {len(df_out)} tickers | {elapsed}s | "
        f"RESERVE={reserve} WATCH={watch} NO_TRADE={no_trade_ct} EXPIRED={expired_ct} | "
        f"zp={zp_nonzero} vs={vs_nonzero} bias_up={bias_up} bias_down={bias_down} | "
        f"news={news_nonzero} fund={fund_nonzero} inv={inv_nonzero} | "
