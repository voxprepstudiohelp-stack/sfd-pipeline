...
# Layer 2.6 Patch V4.1 — sector priority 통합
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from layers.layer2_6_sector_priority import apply_sector_priority
...
def run():
    ...
    for idx, (_, row) in enumerate(target_df.iterrows(), 1):
        ...
        records.append(apply_sector_priority(rec))

        if ok <= 20 or ok % 10 == 0:
            print(f"  [{idx:3d}] ✅ {ticker} {name:12s} | PER={per} PBR={pbr} | {f_score:3d}pt")

    print(f"\n{'='*60}")
    print(f"[RESULT] OK={ok} | FAIL={fail} | Coverage {ok/total*100:.1f}%")
    print(f"{'='*60}\n")
